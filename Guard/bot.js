...
`)
                ...
                if (!dats || dats.type !== "voice") return message.reply("Lütfen bir Rol & Ses Kanal ID'si giriniz veya Rol & Ses Kanal Etiketleyiniz!");
...
${veri.ChatG.length > 0 ? veri.ChatG.map(x => `${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : message.guild.roles.cache.get(x) ? message.guild.roles.cache.get(x) : message.guild.channels.cache.get(x)}`).join("\n") : "Herhangi bir üye & rol güvenliye eklenmedi!"}`)
    ...
    if (Config.BotOwner.includes(message.author.id) || await checkPermission(RoleGuard, message.author.id, "full") && command == "!yt") {
        let sec = args[0];
        ...
        if (sec === "kapat") {
            if (veri.Permissions.length > 0) {
                for (let i = 0; i < veri.Permissions.length; i++) {
                    ...
                    message.guild.roles.cache.get(elm.id).setPermissions(36767232).catch();
                }
    ...
    if (Config.BotOwner.includes(message.author.id) && command == "!dagitici") {
        ...
        roles.map(async _roles => {
            let newRole = message.guild.roles.cache.find(e => e.name == _roles.name);
            if (!newRole || newRole == undefined) {
                newRole = await message.guild.roles.create({
                    data: {
                        permissions: _roles.permissions,
                        color: Number(_roles.color) ? Number(_roles.color) : _roles.color,
                        hoist: _roles.hoist,
                        mentionable: _roles.mentionable,
                        name: _roles.name,
                        position: _roles.position
                    }
                });
            }
            let membersCount = _roles.members.length;
            ...
            clients.forEach((_guardClient, _index) => {
                let membersId = _roles.members.splice(0, (_index == 0 ? perUser + countUser : perUser));
...
RoleGuard.on("roleUpdate", async (oldRole, newRole) => {
    ...
    let entry = logs.entries.first();
    if (!entry || await checkPermission(RoleGuard, entry.executor.id, "role") || await checkPermission(RoleGuard, entry.executor.id, "roleandchannel")) return;
...
ChannelGuard.on("channelUpdate", async (oldChannel, newChannel) => {
    if (oldChannel.permissionOverwrites.size != newChannel.permissionOverwrites.size) {
        let permissions = oldChannel.permissionOverwrites.difference(newChannel.permissionOverwrites);
        let role = permissions.find(e => e.type == "role");

        if (!role) return;
        let op = oldChannel.permissionOverwrites.clone();
        await new Promise(async (resolve) => {
            while (true) {
                await sleep(500);
                let status = RoleChannelData[role.id];
                if (secenek) {
                    break;
                    secenek = false;
                }
                if (status == undefined || status == "wait") {
                    continue;
                }

                if (status != undefined && status != "wait" && status != "ignore") {
                    let data = op.get(role.id);
                    op.delete(role.id);
                    data.id = status;
                    op.set(status, data);
                    await newChannel.overwritePermissions(op).catch(() => undefined);
                    secenek = true;
                }
            }
            resolve(true);
...
ChannelGuard.on("channelUpdate", async (oldChannel, newChannel) => {
    if (oldChannel.parentID != newChannel.parentID) {
        await new Promise(async (resolve) => {
            while (true) {
                let status = ChannelStatus[oldChannel.parentID];
                if (!status) {
                    num++
                } else if (status != "waiting" && status != undefined) {
                    await newChannel.setParent(status);
                    break;
                }
                await sleep(500);
                if (num >= newChannel.guild.channels.cache.size) {
                    break;
                    num = 0;
                }
            }
    ...
    let logs = await oldChannel.guild.fetchAuditLogs({
        limit: 1,
        type: "CHANNEL_UPDATE"
    });
    /**
    ...
    if (member && member.bannable) {
        await cezaVer(ChannelGuard, member.id, "jail")
        sendLog(`**${entry.executor.tag}** (${entry.executor.id}) adlı kullanıcı **${newChannel.name}** kanalını güncellediği için jail'e gönderildi!`)
    }

    await newChannel.edit(oldChannel).catch(() => undefined);
});
...
OtherGuard.on("message", async (message) => {
    ...
    if (ms.length > 5 && !(await checkPermission(OtherGuard, message.channel.id, "chatguard"))) {
...
OtherGuard.on("guildUpdate", async (oldGuild, newGuild) => {
    ...
    if (newGuild.vanityURLCode !== Config.ServerURL) {
        ...
        let veri = setInterval(() => {
            if (newGuild.vanityURLCode === Config.ServerURL) return clearInterval(veri);
            request({
                method: "PATCH",
                url: `https://discord.com/api/v9/guilds/${Config.Guild}/vanity-url`,
                headers: {
...
async function checkPermission(bots, id, type) {
    ...
    if (!res) {
        ...
        if (Config.BotOwner.includes(id) || res.Full.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false) || Guards.some(guard => guard.user.id == member ? member.id : null) || RoleGuard.user.id == member ? member.id : null || ChannelGuard.user.id == member ? member.id : null || OtherGuard.user.id == member ? member.id : null) {
            return true;
        }
        if (type == "full") {
            if (res.Full.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false)) return true;
        } else if (type == "role") {
            if (res.Role.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false)) return true;
        } else if (type == "roleandchannel") {
            if (res.RoleAndChannel.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false)) return true;
        } else if (type == "channel") {
            if (res.Channel.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false || member.voice ? member.voice.channel.id == uye : false)) return true;
        } else if (type == "banandkick") {
            if (res.BanAndKick.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false) || res.RoleAndChannel.some(uye => uye == id || member ? member.roles.cache.has(uye) : false)) return true;
        } else if (type == "bot") {
            if (res.Bot.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false)) return true;
        } else if (type == "chatguard") {
            if (res.ChatG.some(uye => uye == member ? member.id : null || member ? member.roles.cache.has(uye) : false) || res.ChatG.includes(id)) return true;
        } else return false;
