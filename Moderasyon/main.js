...
client.on("message", async message => {
  ...
  if (message.member.roles.cache.some(rol => komut.roller.includes(rol.id)) || komut.kisiler.includes(message.author.id) || message.member.permissions.has(8)) {
...
client.permAyar = async function (userID, guildID, type) {
  ...
  let enAltYetki = data.EnAltYetkiliRol
  ...
  let sabitpermler = member.permissions.has(8) ||
    ...
    member.roles.cache.some(rol => TUMYTLER.includes(rol.id));
